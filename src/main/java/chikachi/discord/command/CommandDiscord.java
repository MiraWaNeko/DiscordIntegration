        ...
        switch (commandName) {
            ...
            case "link":
                if (!Configuration.getConfig().discord.allowLinking) {
                    sender.addChatMessage(new TextComponentString(MinecraftFormattingCodes.DARK_RED + "Linking is not enabled"));
                    break;
                }

                if (!(sender instanceof EntityPlayer)) {
                    sender.addChatMessage(new TextComponentString(MinecraftFormattingCodes.DARK_RED + "You need to be a player"));
                    break;
                }
                ...
                if (argsList.size() == 0) {
                    sender.addChatMessage(new TextComponentString(MinecraftFormattingCodes.DARK_RED + "Missing code"));
                    break;
                }
                ...
                if (requestOptional.isPresent()) {
                    ...
                    if (request.hasExpired()) {
                        sender.addChatMessage(new TextComponentString(MinecraftFormattingCodes.DARK_RED + "Linking request has expired"));
                        break;
                    }

                    request.executeLinking(minecraftUUID);
                    sender.addChatMessage(new TextComponentString(MinecraftFormattingCodes.GREEN + "Linked"));
                } else {
                    sender.addChatMessage(new TextComponentString(MinecraftFormattingCodes.DARK_RED + "Linking request not found"));
                    break;
                }
                break;
            case "unlink":
                ...
                if (Configuration.getLinking().getDiscordId(minecraftUUID) == null) {
                    sender.addChatMessage(new TextComponentString(MinecraftFormattingCodes.DARK_RED + "You aren't linked"));
                    break;
                }

                Configuration.getLinking().removeLink(minecraftUUID);
                sender.addChatMessage(new TextComponentString(MinecraftFormattingCodes.GREEN + "Unlinked"));
                break;
            case "tps":

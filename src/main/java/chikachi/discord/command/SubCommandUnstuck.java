...
public class SubCommandUnstuck {
    public static void execute(ICommandSender sender, ArrayList<String> args) {
        if (args.size() == 0) {
            sender.addChatMessage(new TextComponentString("Missing user"));
            return;
        }
        ...
        if (player != null) {
            ...
            if (playerProfile == null || !playerProfile.isComplete()) {
                sender.addChatMessage(new TextComponentString("Player not found"));
                return;
            }
            ...
            if (playerData == null) {
                sender.addChatMessage(new TextComponentString("Player not found on server"));
                return;
            }
        ...
        sender.addChatMessage(new TextComponentString("Player sent to spawn"));
    }
}

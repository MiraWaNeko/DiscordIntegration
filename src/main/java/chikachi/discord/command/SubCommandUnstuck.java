...
public class SubCommandUnstuck {
    public static void execute(ICommandSender sender, ArrayList<String> args) {
        ...
        BlockPos spawnpoint = overworld.getSpawnCoordinate();
        IBlockState blockState = overworld.getBlockState(spawnpoint);

        while (blockState.getBlock().isOpaqueCube()) {
        ...
        EntityPlayerMP player = minecraftServer.getConfigurationManager().getPlayerByUsername(username);
